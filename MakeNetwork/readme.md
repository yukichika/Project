...
Webページ間の類似度をコサイン類似度（正規化）・ユークリッド距離（正規化）で算出．  

・check_distance_distribution.py(python2)  
可視化するWebページ間の類似度の分布（自分自身は除く）を保存．  
コサイン類似度・コサイン類似度(正規化)・ユークリッド距離・ユークリッド距離(正規化)・指数で正規化したユークリッド距離を使用．  

・wikipedia_distance.py(python3)  
doc2vecのベクトルの類似度をコサイン類似度・ユークリッド距離・指数で正規化したユークリッド距離で算出．  
※可視化するWebページは使っていないため，テスト用（Wikipediaを使用）  

・wikipedia_cal.py(python3)  
doc2vecのベクトルの演算をコサイン類似度・ユークリッド距離・指数で正規化したユークリッド距離で算出．  
※可視化するWebページは使っていないため，テスト用（Wikipediaを使用）  
