...
def draw(G, pos=None, ax=None, hold=None, **kwds):
	...
	if ax is None:
		if cf._axstack() is None:
			ax = cf.add_axes((0, 0, 1, 1))#描画範囲をここで決定．引数のタプルはleft,bottom,width,heightの要素らしいが全然そうなっていないような．ここを調整すると軸を出せる．
		else:
			ax = cf.gca()
...
def draw_networkx_nodes(G, pos,
	...
	if pick_func!=None:
		node_collection = ax.scatter(xy[:, 0], xy[:, 1],
									 ...
									 picker=True)
		cf = ax.get_figure()
		cf.canvas.mpl_connect("pick_event",pick_func)
...
def draw_networkx_edges(G, pos,
	...
	if G.is_directed() and arrows:
		...
		if ORG_ARROW==True:
			...
			#"""ここから改変"""
			for src, dst in edge_pos:
			    ax.annotate("",xy=dst,xytext=src,xycoords="data",textcoords="data",arrowprops=dict(arrowstyle="-|>,head_length=0.8,head_width=0.4",connectionstyle="arc3"))
